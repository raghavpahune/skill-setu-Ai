...
def _build_context(
    ...
    try:
        ...
        skills_map = {s.get("id"): s for s in skills if isinstance(s, dict) and s.get("id")}
        pre_gaps = compute_gaps(
            district=None,
            is_demo=is_demo,
            jobs=jobs,
            job_skills=job_skills,
            courses=courses,
            course_skills_data=course_skills,
            skills_map=skills_map,
        )
        context: dict[str, Any] = {
            "query_type": "general_overview",
            "top_skill_gaps": pre_gaps[:10],
        ...
        if queried_skill_info:
            if queried_skill_info["type"] == "indexed" and (is_demo or (skills_ok and jobs_ok and job_skills_ok and course_skills_ok)):
                ...
                all_gaps = compute_gaps(
                    district=focused_district.get("district") if focused_district else None,
                    is_demo=is_demo,
                    jobs=jobs,
                    job_skills=job_skills,
                    courses=courses,
...
async def handle_question(
    ...
    if not is_demo_mode:
        ...
        try:
            answer = await asyncio.wait_for(provider.generate(question, context), timeout=14.0)
            ...
            logger.warning(f"[Copilot] Live generation timed out for role={role}")
            return {
                "answer": "AI Copilot live inference timed out while connecting to the AI model. In Real Data mode, synthetic factual fallbacks are disabled to prevent inaccurate labour market intelligence. Please try again in a few moments.",
                "role": role,
                "student_id": student_id,
                "demo_mode": False,
                "data_grounded": bool(context),
                "model": "Real Data Service (Timeout)",
                "provenance_label": "⚠️ Service Timeout",
            }
        except Exception as e:
            err_msg = str(e)
